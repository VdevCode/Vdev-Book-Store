...
const EditBook = () => {
  ...
  const {bookTitle, authorName,imageURL,categoryName,bookPDFURL,bookDescription} = useLoaderData()
  ...
  return (
    <div className="px-4 my-12 z-0">
      <h2 className="mb-8 text-xl font-boild">Chỉnh sửa</h2>

      <form onSubmit={handleUpdate} className="flex lg:w-[100%] flex-col flex-wrap gap-4">
        {/* form1 */}
          ...
          <div className="w-full">
            ...
            >
              {bookCategories.map((option, index) => (
                <option key={index} defaultValue={categoryName}>{option}</option>
              ))}
            </Select>
          </div>
        </div>
